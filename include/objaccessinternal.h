...
#define READ_UNS32(objDict, index, subIndex)                    \
    (objDict[index].pSubindex[subIndex].bAccessType != CONST    \
         ? *(UNS32*) objDict[index].pSubindex[subIndex].pObject \
         : *(const CONSTSTORE UNS32*) objDict[index].pSubindex[subIndex].pObjectConst)

#define READ_UNS16(objDict, index, subIndex)                    \
    (objDict[index].pSubindex[subIndex].bAccessType != CONST    \
         ? *(UNS16*) objDict[index].pSubindex[subIndex].pObject \
         : *(const CONSTSTORE UNS16*) objDict[index].pSubindex[subIndex].pObjectConst)

#define READ_UNS8(objDict, index, subIndex)                    \
    (objDict[index].pSubindex[subIndex].bAccessType != CONST   \
         ? *(UNS8*) objDict[index].pSubindex[subIndex].pObject \
         : *(const CONSTSTORE UNS8*) objDict[index].pSubindex[subIndex].pObjectConst)

#define IS_NULL(objDict, index, subIndex)                     \
    (objDict[index].pSubindex[subIndex].bAccessType != CONST  \
         ? objDict[index].pSubindex[subIndex].pObject == NULL \
         : objDict[index].pSubindex[subIndex].pObjectConst == NULL)

#define WRITE_UNS32(objDict, index, subIndex, value) (*((UNS32*) objDict[index].pSubindex[subIndex].pObject) = value)

#define WRITE_UNS16(objDict, index, subIndex, value) (*((UNS16*) objDict[index].pSubindex[subIndex].pObject) = value)

#define WRITE_UNS8(objDict, index, subIndex, value) (*((UNS8*) objDict[index].pSubindex[subIndex].pObject) = value)
