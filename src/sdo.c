 ...
 ** @return
 **/
INLINE CanOpen_SDO_StateTypeDef _writeNetworkDict(CO_Data*      d,
                                                  UNS8          nodeId,
                                                  UNS16         index,
                                                  UNS8          subIndex,
                                                  UNS32         count,
 ...
 ** @return
 **/
INLINE CanOpen_SDO_StateTypeDef _readNetworkDict(CO_Data*      d,
                                                 UNS8          nodeId,
                                                 UNS16         index,
                                                 UNS8          subIndex,
                                                 UNS8          dataType,
...
void SDOTimeoutAlarm(CO_Data* d, UNS32 id)
{
    ...
    /* Sending a SDO abort */
    sendSDOabort(d,
                 d->transfers[id].whoami,
                 d->transfers[id].CliServNbr,
...
#define RestartSDO_TIMER(id)                               \
    ...
    if (d->transfers[id].timer != TIMER_NONE)              \
    {                                                      \
        StopSDO_TIMER(id) StartSDO_TIMER(id)               \
    }
...
UNS8 SDOtoLine(CO_Data* d, UNS8 line, UNS32 nbBytes, UNS8* data)
{
...
#ifdef SDO_DYNAMIC_BUFFER_ALLOCATION
    {
        ...
        if ((d->transfers[line].offset + nbBytes) > SDO_MAX_LENGTH_TRANSFER)
        {
            ...
            {
                UNS8* newDynamicBuffer =
                    (UNS8*) realloc(d->transfers[line].dynamicData,
                                    d->transfers[line].dynamicDataSize + SDO_DYNAMIC_BUFFER_ALLOCATION_SIZE);
                if (newDynamicBuffer == NULL)
                {
                    MSG_ERR(0x1A15,
                            "SDO reallocating dynamic buffer failed, size",
                            d->transfers[line].dynamicDataSize + SDO_DYNAMIC_BUFFER_ALLOCATION_SIZE);
                    return 0xFF;
                }
...
UNS8 failedSDO(CO_Data* d, UNS8 CliServNbr, UNS8 whoami, UNS16 index, UNS8 subIndex, UNS32 abortCode)
{
    ...
    if (!err)
    { /* If a line on use have been found.*/
...
UNS8 getSDOlineOnUse(CO_Data* d, UNS8 CliServNbr, UNS8 whoami, UNS8* line)
{
    ...
    for (i = 0; i < SDO_MAX_SIMULTANEOUS_TRANSFERS; i++)
    {
        if ((d->transfers[i].state != SDO_RESET) && (d->transfers[i].state != SDO_ABORTED_INTERNAL) &&
            (d->transfers[i].CliServNbr == CliServNbr) && (d->transfers[i].whoami == whoami))
        {
            if (line)
                *line = i;
            return 0;
...
UNS8 getSDOlineToClose(CO_Data* d, UNS8 CliServNbr, UNS8 whoami, UNS8* line)
{
    ...
    for (i = 0; i < SDO_MAX_SIMULTANEOUS_TRANSFERS; i++)
    {
        if ((d->transfers[i].state != SDO_RESET) && (d->transfers[i].CliServNbr == CliServNbr) &&
            (d->transfers[i].whoami == whoami))
        {
            if (line)
                *line = i;
...
UNS8 proceedSDO(CO_Data* d, Message* m)
{
    ...
    case 0:
        ...
        { /* if CLIENT */
            ...
            /* Reset the wathdog */
            RestartSDO_TIMER(line) index = d->transfers[line].index;
            subIndex                     = d->transfers[line].subIndex;
            /* test of the toggle; */
            if (d->transfers[line].toggle != getSDOt(m->data[0]))
            {
            ...
            if (getSDOc(m->data[0]))
            {
                ...
                StopSDO_TIMER(line) d->transfers[line].state = SDO_FINISHED;
    ...
    case 2:
        ...
        else
        {
            ...
            { /* If SDO expedited */
                ...
                StopSDO_TIMER(line) d->transfers[line].count = nbBytes;
    ...
    case 4:
        abortCode =
            (UNS32) m->data[4] | ((UNS32) m->data[5] << 8) | ((UNS32) m->data[6] << 16) | ((UNS32) m->data[7] << 24);
        /* Received SDO abort. */
        if (whoami == SDO_SERVER)
        {
            if (!err)
        ...
        { /* If I am CLIENT */
            if (!err)
            {
                ...
                StopSDO_TIMER(line) d->transfers[line].state = SDO_ABORTED_RCV;
                d->transfers[line].abortCode                 = abortCode;
    ...
    case 5: /* Command specifier for data transmission - the client or server is the data producer */
        ...
        { /* Server block upload */
            if (SubCommand == SDO_BCS_INITIATE_UPLOAD_REQUEST)
            {
                ...
                data[7]                    = (UNS8) (nbBytes >> 24);
                MSG_WAR(0x3A9A,
                        "SDO. Sending normal block upload initiate response defined at index 0x1200 + ",
                        nodeId);
                sendSDO(d, whoami, CliServNbr, data);
            }
            else if (SubCommand == SDO_BCS_END_UPLOAD_REQUEST)
            {
                ...
                {
                    MSG_ERR(0x1AA1,
                            "SDO error : Received block upload request for unstarted trans. index 0x1200 + ",
                            CliServNbr);
                    failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
                    return 0xFF;
                }
            ...
            else if ((SubCommand == SDO_BCS_UPLOAD_RESPONSE) || (SubCommand == SDO_BCS_START_UPLOAD))
            {
                ...
                {
                    MSG_ERR(0x1AA1,
                            "SDO error : Received block upload response for unstarted trans. index 0x1200 + ",
                            CliServNbr);
                    failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
                    return 0xFF;
                }
                ...
                if (SubCommand == SDO_BCS_UPLOAD_RESPONSE)
                {
                    ...
                    {
                        data[0] =
                            (UNS8) ((6 << 5) | ((d->transfers[line].endfield) << 2) | SDO_BSS_END_UPLOAD_RESPONSE);
                        for (i = 1; i < 8; i++)
                            data[i] = 0;
                        MSG_WAR(0x3AA5,
                                "SDO. Sending block END upload response defined at index 0x1200 + ",
                                CliServNbr);
                        sendSDO(d, whoami, CliServNbr, data);
                        break;
                    }
                    ...
                    { /* Bad AckSeq reveived (too high) */
                        MSG_ERR(0x1AA1,
                                "SDO error : Received upload response with bad ackseq index 0x1200 + ",
                                CliServNbr);
                        failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
                        return 0xFF;
                    }
        ...
        { /* if CLIENT (block download) */
            if ((SubCommand == SDO_BSS_INITIATE_DOWNLOAD_RESPONSE) || (SubCommand == SDO_BSS_DOWNLOAD_RESPONSE))
            {
                ...
                {
                    MSG_ERR(0x1AAA,
                            "SDO error : Received response for unknown block download request from node id",
                            nodeId);
                    failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
                    return 0xFF;
                }
                ...
                subIndex = d->transfers[line].subIndex;
                /* Reset the watchdog */
                RestartSDO_TIMER(line) if (SubCommand == SDO_BSS_INITIATE_DOWNLOAD_RESPONSE)
                {
                    if (m->data[4] > 127)
                    {
                ...
                else
                {
                    ...
                    {
                        data[0] =
                            (UNS8) ((6 << 5) | ((d->transfers[line].endfield) << 2) | SDO_BCS_END_DOWNLOAD_REQUEST);
                        for (i = 1; i < 8; i++)
                            data[i] = 0;
                        MSG_WAR(0x3AA5,
                                "SDO. Sending block END download request defined at index 0x1200 + ",
                                CliServNbr);
                        sendSDO(d, whoami, CliServNbr, data);
                        break;
                    }
                    ...
                    { /* Bad AckSeq reveived (too high) */
                        MSG_ERR(0x1AA1,
                                "SDO error : Received upload segment with bad ackseq index 0x1200 + ",
                                CliServNbr);
                        failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
                        return 0xFF;
                    }
    ...
    case 6: /* Command specifier for data reception - the client or server is the data consumer */
        ...
        { /* Server block download */
            if (err)
            {
                ...
                if ((m->data[0]) & 2) /* if data set size is indicated */
                    d->transfers[line].objsize = (UNS32) m->data[4] + (UNS32) m->data[5] * 256 +
            ...
            else if (d->transfers[line].rxstep == RXSTEP_STARTED)
            {
                ...
                RestartSDO_TIMER(line) SeqNo = m->data[0] & 0x7F;
                if (m->data[0] & 0x80)
                { /* Last segment ? */
            ...
            { /* endphase */
                ...
                {
                    MSG_ERR(0x1AAD,
                            "SDO error block download : Received wrong subcommand - index 0x1200 + ",
                            CliServNbr);
                    failedSDO(d, CliServNbr, whoami, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
                    return 0xFF;
                }
                RestartSDO_TIMER(line) NbBytesNoData = (UNS8) ((m->data[0] >> 2) & 0x07);
                /* Store the data in the transfer structure. */
                ...
                { /* If size was indicated in the initiate request */
        ...
        { /* if CLIENT (block upload) */
            ...
            RestartSDO_TIMER(line) if (d->transfers[line].rxstep == RXSTEP_INIT)
            {
                                                  ...
                                                  UNS8          useBlockMode)
{
    ...
    {
        MSG_ERR(0x1AC3,
                "SDO error : request for more than SDO_MAX_LENGTH_TRANSFER bytes to transfer to node : ",
                nodeId);
        return 0xFF;
    }
#endif
 ...
 ** @return
 **/
CanOpen_SDO_StateTypeDef writeNetworkDictCallBack(CO_Data*      d,
                                                  UNS8          nodeId,
                                                  UNS16         index,
                                                  UNS8          subIndex,
                                                  UNS32         count,
