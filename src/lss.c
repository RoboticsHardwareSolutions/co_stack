...
#    define getLSSIdent(msg) \
        (((UNS32) msg->data[4] << 24) | ((UNS32) msg->data[3] << 16) | (msg->data[2] << 8) | (msg->data[1]))
...
#    define StartLSS_SDELAY_TIMER()                                                             \
        {                                                                                       \
            ...
            d->lss_transfer.timerSDELAY = SetAlarm(d,                                           \
                                                   0,                                           \
                                                   &LssAlarmSDELAY,                             \
                                                   MS_TO_TIMEVAL(d->lss_transfer.switchDelay),  \
                                                   MS_TO_TIMEVAL(d->lss_transfer.switchDelay)); \
        }
...
#    endif
        ...
        break;
    case LSS_SM_SELECTIVE_RESP: /* Switch Mode Selective response */
        if (d->lss_transfer.command != LSS_SM_SELECTIVE_VENDOR && d->lss_transfer.command != LSS_SM_SELECTIVE_PRODUCT &&
            d->lss_transfer.command != LSS_SM_SELECTIVE_REVISION && d->lss_transfer.command != LSS_SM_SELECTIVE_SERIAL)
            goto ErrorProcessMaster;
        break;
    case LSS_IDENT_NON_CONF_SLAVE: /* LSS identify non-configured remote slave */
        if (d->lss_transfer.command != LSS_IDENT_REMOTE_NON_CONF)
