        ...
        if (d->NMTable[nodeId] != Unknown_state)
        {
            ...
            for (index = (UNS8) 0x00; index < *d->ConsumerHeartbeatCount; index++)
            {
                ConsumerHeartBeat_nodeId =
                    (UNS8) (((d->ConsumerHeartbeatEntries[index]) & (UNS32) 0x00FF0000) >> (UNS8) 16);
                if (nodeId == ConsumerHeartBeat_nodeId)
                {
                    ...
                    d->ConsumerHeartBeatTimers[index] =
                        SetAlarm(d, index, &ConsumerHeartbeatAlarm, MS_TO_TIMEVAL(time), 0);
                }
            }
        }
    }
