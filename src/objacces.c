...
{ /* DO NOT USE MSG_ERR because the macro may send a PDO -> infinite
    ...
    *pDataType = ptrTable->pSubindex[bSubindex].bDataType;
        ...
        if (ptrTable->pSubindex[bSubindex].bAccessType == CONST)
    {
        ...
        {
            const CONSTSTORE char* dp =
                *(const CONSTSTORE char* const CONSTSTORE*) ptrTable->pSubindex[bSubindex].pObjectConst;
                  ...
                  UNS8     endianize)
{
    ...
    if (checkAccess &&
        (ptrTable->pSubindex[bSubindex].bAccessType == RO || ptrTable->pSubindex[bSubindex].bAccessType == CONST))
    {
