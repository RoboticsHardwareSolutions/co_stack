...
UNS8 buildPDO(CO_Data* d, UNS8 numPdo, Message* pdo)
{
    ...
    MSG_WAR(0x3009, "  PDO CobId is : ", READ_UNS32(d->objdict, d->firstIndex->PDO_TRS + numPdo, 1));
    ...
    do
    {
        ...
        if (Size && ((offset + Size) <= 64))
        {
            ...
            {
                MSG_ERR(0x1013, " Couldn't find mapped variable at index-subindex-size : ", (UNS32) (mappingParameter));
                return 0xFF;
            }
                    ...
                    if (d->RxPDO_EventTimers)
                    {
                        ...
                        if (EventTimerDuration)
                        {
                            ...
                            d->RxPDO_EventTimers[numPdo] =
                                SetAlarm(d, numPdo, d->RxPDO_EventTimers_Handler, MS_TO_TIMEVAL(EventTimerDuration), 0);
                        }
                    }
                    return 0;
        ...
        if (offsetObjdict)
            while (offsetObjdict <= lastIndex)
            {
                ...
                case state4: /* check transmission type */
                {
                    ...
                    else if (transmissionType == TRANS_EVENT_PROFILE || transmissionType == TRANS_EVENT_SPECIFIC)
                    {
              ...
              UNS8  DestBigEndian)
{
    ...
    while (NbBits > 0)
    {
        ...
        UNS8 Aligned = (UNS8) (Vect > 0 ? *SrcByteIndex << Vect : *SrcByteIndex >> -Vect);
        ...
        UNS8 Mask = (UNS8) ((0xff << (DestBitIndex + BitsToCopy)) | (0xff >> (8 - DestBitIndex)));
...
UNS8 sendOnePDOevent(CO_Data* d, UNS8 pdoNum)
{
    ...
    if (d->PDO_status[pdoNum].last_message.cob_id == pdo.cob_id && d->PDO_status[pdoNum].last_message.len == pdo.len &&
    ...
    if (offsetObjdict)
    {
        ...
        while (offsetObjdict <= lastIndex)
        {
            ...
            case state3:
                ...
                if (isSyncEvent && transmissionType >= TRANS_SYNC_MIN && transmissionType <= TRANS_SYNC_MAX &&
                    (++d->PDO_status[pdoNum].transmit_type_parameter == transmissionType))
                {
                    /*Reset count of SYNC */
                    d->PDO_status[pdoNum].transmit_type_parameter = 0;
                    MSG_WAR(0x3964, "  PDO is on SYNCHRO. Trans type : ", transmissionType);
                    memset(&pdo, 0, sizeof(pdo));
                    /*{
                ...
                else if (isSyncEvent && transmissionType == TRANS_RTR_SYNC)
                {
                    ...
                    /* If transmission on Event and not inhibited, check for changes */
                         ...
                         (!isSyncEvent &&
                          (transmissionType == TRANS_EVENT_PROFILE || transmissionType == TRANS_EVENT_SPECIFIC) &&
                          !(d->PDO_status[pdoNum].transmit_type_parameter & PDO_INHIBITED)))
                {
...
UNS32
TPDO_Communication_Parameter_Callback(CO_Data* d, UNS16 wIndex, UNS8 bSubindex)
{
    /* If PDO are actives */
    if (d->CurrentCommunicationState.csPDO)
        switch (bSubindex)
